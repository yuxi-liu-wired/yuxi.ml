...
It covers: calculus of variations, Lagrangian, Legendre transform, Hamiltonian, Hamilton's principal function, Hamilton--Jacobi equation, the particle-wave duality, canonical transforms, infinitesimal generator, contact geometry, symplectic geometry, Schrödinger's equation, Hamilton's geometric optics, action-angle variables, Noether's theorem, adiabaticity, old quantum theory, Bohr--Sommerfeld quantization, Einstein--Brillouin--Keller quantization.

It does not cover: Poisson bracket, canonical perturbation theory.
...
If you have seen it before, you will recognize this: It is a contact structure on the 1-jet manifold. In this sense, Lagrangian mechanics *is* [contact geometry](https://en.wikipedia.org/wiki/Contact_geometry). Although, as we commented in the [contact-geometric interpretation of thermodynamics](https://yuxi.ml/essays/posts/equilibrium-thermoeconomics/#contact-geometry), this does not mean that we can just import intuition about contact geometry into Lagrangian mechanics, because we have even less intuition about contact geometry than about Lagrangian mechanics!

So one may object "Lagrangian mechanics is reduced to contact geometry... but what is contact geometry?" My answer is that contact geometry simply *is*. It is not supposed to be a generator of intuitions. Instead, it is a common language that bridges between intuitions. By casting thermodynamics, economics, Lagrangian mechanics, etc, into the language of contact geometry, we would then be able to translate intuition from one field to another field. Saying that "Lagrangian mechanics is contact geometry" is not telling you an intuitive way to see Lagrangian mechanics, but rather, an intuitive way to see contact geometry.

Lifting a curve in configuration space-time is simply finding the unique curve in the 1-jet manifold that is tangent to the field of planes, and projects down to that same curve, so you can imagine looking down from the top of the curve in $J^1$ and see just the original curve in configuration spacetime. In equations, a trajectory $(t, \gamma(t))$ is lifted to $(t, \gamma(t), \dot \gamma(t))$, and the lifted trajectory has tangent vector $(1, \dot \gamma(t), \ddot \gamma(t))$.

![A curve in the configuration space-time $\R^2$ is lifted to a curve in the 1-jet manifold $\R^3$. The lifted curve is tangent to the field of planes at every point, and projects down to the original curve. There is only one possible way to lift it. If you look down from the top, you see the original curve in $\R^2$](todo)
...
![Lifting multiple curves through the same point.](todo)
...
Given a Lagrangiang $L: J^1 \to \R$, a system at point $(t, q)$ can go on evolving in any direction. We need to know its velocity $v$ before we can apply the EL equations to solve for its acceleration, and thus calculate its entire past and future. In the 1-jet manifold, given the Lagrangian $L : J^1 \to \R$, we can calculate an acceleration function $a: J^1 \to \R^N$ using the EL equation, then we install that acceleration function into $J^1$ as part of a vector field: at each $(t, q, v) \in J^1$, we install the time-evolution vector field $\Gamma = (1, v, a)$, which satisfies
...
![A single trajectory $\Gamma$ in 1-jet manifold, going from $(t_0, q_0, v_0)$ to $(t_1, q_1, v_1)$. The integral $\int_\Gamma Ldt$ is obtained by counting how many sheets it pierces.](todo)
...
![When the 1-form $Ldt$ is not trivial, varying $\Gamma$ can change $\int Ldt$ a lot. When the 1-form is exact, varying $\Gamma$ does not change $\int Ldt$.](todo)
...
![$dF$ is exact, and made of vertical curvy sheets, so $\int_\Gamma dF= F(t_1, q_1) - F(t_0, q_0)$ does not depend on the trajectory of $\Gamma$, only its starting and ending vertical lines.](todo){width=50%}

If only we can write $L'dt = Ldt + dF$, then we would obtain our new Lagrangian! Alas, this is in general impossible: the infinitesimal sheets of $(L'-L)dt$ are parallel to $dt$, which is strictly perpendicular to the t-axis, but the infinitesimal sheets of $dF = \partial_t F dt + \sum_i \partial_{q_i} F dq_i$ may not be.
...
$$
(T(t, q), Q(t, q), D_t Q - (D_t T) v), \quad D_t := \partial_t + \braket{v, \partial_q}
$$ {#eq-pr-1-manifold}
...
![An infinitesimal square in $J^1$ is dragged by $(\epsilon T, \epsilon Q)$, resulting in a translation and a rotation. The rotation causes it to misfit its new home, and must be translated again along the vertical direction by $\epsilon R$.](todo)
...
### Bonus: Velocity-dependent transformation

Or rather, why we don't have velocity-dependent transformations.
...
This is fundamentally why, though point transformations are everywhere in Lagrangian mechanics, velocity-dependent (or higher-order-dependent) transformations are unknown. Some have tried, but I think it's not worth the effort.
...
Now, suppose we have found some functions $P, Q, K, F$ on phase space-time, satisfying
...
There are *two* ways to interpret this equation wrongly. One way to mess up is to differentiate $dq, dQ$ *across time*. No, they must be differentiated *within each slice of time*. Another way to mess up is to *fail* to differentiate $dF$ across time. No, it must be differentiated across time. So if we are to be maximally precise, we should do something like this. First, we impose an arbitrary, but time-independent reference coordinate system $x_1, \dots, x_N, y_1, \dots, y_N$, such that $\partial_t x = 0, \partial_t y = 0$. Then, we write maximally explicitly:

$$
\begin{aligned}
&\sum_i \sum_j p_i (\partial_{x_j}q_i dx_j + \partial_{y_j}q_i dy_j) - Hdt \\
= &\sum_i \sum_j P_i (\partial_{x_j}Q_i dx_j + \partial_{y_j}Q_i dy_j) - Kdt 
+ \sum_j (\partial_{x_j}F dx_j + \partial_{y_j}F dy_j) + \partial_t F dt
\end{aligned}
$$
...
for some $E\in \R$, then $S(t, q) = W(q) - Et$ is a solution to the HJE. This is easy to interpret using 

We might naively think, by analogy with Fourier transform, that any solution to the HJE is a linear combination of functions of the form $S(t, q) = \int (W_E(q) - Et)dE$, but this is not true, since the HJE is nonlinear. Nevertheless, solutions of the form $S(t, q) = W(q) - Et$ are often sufficient for applications.
...
The famed Noether's theorem states that symmetries of the Lagrangian give us conserved quantities of motion. To state this rigorously, we set up some notation:
...
We defined "symmetry of the Lagrangian" by a condition on an integral: $\int_\gamma L dt = \int_{\gamma'} L dt$. Now we reformulate this to a condition at a point, involving $L, Q, T$ and their derivatives.

::: {.callout-note title="Derivation" collapse="false"}
...
Given any physically real trajectory $\gamma_{AB}$ from point $A$ to $B$, we construct a variation of it by applying the infinitesimal transform, obtaining $A \to A + \delta A \to B + \delta B \to B$. Since $\gamma_{AB}$ is physically real, this perturbs the action by only $o(\delta)$.

Because the infinitesimal transformation preserves the action up to $o(\delta)$, the action of $A + \delta A \to B + \delta B$ is the same as the action of $A \to B$ up to $o(\delta)$. So the action of $A \to A + \delta A$ cancels out the action of $B + \delta B \to B$ up to $o(\delta)$. Now take $\delta \to 0$.
...
![The shape of the $K_{wave}$ surface. The $K_{particle}$ surface is topologically the same, and can be obtained from $K_{wave}$ by squashing it *just right*. Figure from [@schaeferElektrodynamikUndOptik1949, page 485, figure 128]](figure/schaefer_1949_fig_128.png)
...
Begin by inspecting the HJE $\partial_t S + \frac{\|\nabla_q S\|^2}{2m} + V = 0$. It is not a linear differential equation, but all length-preserving rotations are linear, so this tells us that something is wrong -- $S$ cannot be something fundamental. Well, either by a good guess, or by watching animated plots of $S(t, q)$ over time, you could be reminded of a water wave, and so guess that we might be able to find a linear differential equation for $\psi(t, q) = e^{iS(t, q)}$ instead. 
...
$$
\partial_t \psi = \frac{i}{\hbar}\phi\partial_t S, \quad \partial_{q_i}^2 \psi = -\frac{1}{\hbar^2} \phi (\partial_{q_i} S)^2 + \frac{i}{\hbar}\phi \partial_{q_i}^2 S
$$
...
Thus, we remove the inconvenient term, and write down $i\hbar \partial_t \psi = \hat H\psi$, which is the time-dependent Schrödinger equation. Now plug in $\psi = e^{iS/\hbar}$ and show that at $\hbar \to 0$ limit, we recover the HJE.
...
Classical mechanics does not favor the Newtonian, the Lagrangian, and the Hamiltonian. They are theoretically equal, if aesthetically unequal. But why aesthetically unequal? Perhaps because the leakless abstraction leaks. The Hamiltonian is more aesthetic than the Newtonian, because classical Hamiltonian mechanics is a limit of quantum mechanics. The HJE

The Lagrangian is favored over the Newtonian, because special relativity and quantum field theory.

The Hamiltonian is so prevalent in quantum mechanics that when I took an introduction to quantum mechanics, I dreamed about $\hat H$ and $\hat K$. 

 Quantum mechanics leaks into classical mechanics by favoring the Hamiltonian formulation over the Lagrangian formulation, and picking the [Hamilton--Jacobi equation](##hamiltonjacobi-equation) as the "closest approach" to the Schrödinger equation. But what about relativity? How does relativity leak into classical mechanics?

As it turns out, Lagrangian has its revenge. Lagrangian mechanics is really easy to reformulate into a form compatible with relativity! We do the basic example of a particle in a potential field $V(t, q)$. Begin with the classical Lagrangian formulation:

$$
\delta \int (\frac 12 mv^2 - V(t, q)) dt, \quad q(t_0) = q_0, \; q(t_1) = q_1
$$

Add a constant $-mc^2$ to the Lagrangian. It does not change the dynamics. Next, notice that $-mc^2 + \frac 12 mv^2 \approx -mc^2\sqrt{1-(\frac vc)^2}$, and therefore

$$\delta \int (\frac 12 mv^2 - V(t, q)) dt \approx \int (-mc^2 - V(t, q)) \sqrt{1-(\frac vc)^2}dt$$

Now the term $\sqrt{1-(\frac vc)^2}dt = d\tau$ is the proper time, which *is* relativistically invariant, so we obtain the relativistic Lagrangian mechanics of a particle:

$$
\delta \int_\gamma (-mc^2 - V) d\tau, \quad \gamma(\tau_0) = (t_0, q_0), \; \gamma(\tau_1) = (t_1, q_1), \
$$

The crucial trick is that in classical Lagrangian mechanics, the variational constraint $q(t_0) = q_0, \; q(t_1) = q_1$ already treats space and time equally: We require the particle to start at a specific space-and-time, and end at another specific space-and-time. 
...
[@greinerExtendedHamiltonLagrange2010] is an introduction to this approach.

Should you care? Sure, if you like more theory. I find it useless and unaesthetic, because much of the utility and beauty of Lagrangian mechanics is precisely in *not* being imprisoned within the Black Iron Prison of spacetime. Indeed, if you read this introduction [@greinerExtendedHamiltonLagrange2010], you will find that it includes 16 worked examples, but every example either is a generic theory-work, or a single particle in a field. What is the point of generalizing the full machinery of Hamiltonian mechanics in $(2n+1)$-dimensional phase space-time, if you are going to only solve problems in $3+1$ anyway? 
